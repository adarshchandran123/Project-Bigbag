...
router.get("/", async function (req, res, next) {
  if (req.session.loggedIn) {
    ...
    let checkCategoryOfferValidity =
      await productHelpers.checkCategoryOfferValidity();
    ...
    productOfferValidity.map((productdetail) => {
      productHelpers
        .DeleteProductOffer(productdetail._id, productdetail.productName)
        .then(() => {});
    });
...
router.get("/signupOTP_Verify", (req, res) => {
  ...
  res.render("usersignupOTP_Verify", {
    ...
    serverError: req.session.serverError,
  });
...
router.post("/signupOTP", (req, res) => {
  let otp =
    req.body.digit_1 + req.body.digit_2 + req.body.digit_3 + req.body.digit_4;

  var mobileNum = req.session.userRegnumber;

  twilio.verify
    .services("VA716b2ecd593bad2dd3a9c3f305df24c7")
    .verificationChecks.create({ to: "+" + mobileNum, code: otp })
...
router.get("/loginwith_otp", (req, res) => {
  res.render("userLoginwithOTP", {
    errCountryCode_OR_MobNum: req.session.errCountryCode_OR_MobNum,
  });
  req.session.errCountryCode_OR_MobNum = false;
});

router.post("/loginwithOTP", (req, res) => {
  ///original login router
  userHelpers
    .loginWithOTP(req.body)
    .then((response) => {
      mobile1 = req.body.countryCode + req.body.mobile;
      mobileNum = parseInt(mobile1);
...
router.get("/LoginOTP_Verify", (req, res) => {
  ...
  res.render("userLoginOTP_Verify", {
    mobileNum,
    otpError: req.session.otpError,
    serverError: req.session.serverError,
  });
...
router.get("/userForgotPassword", (req, res) => {
  if (req.session.loggedIn) {
    ...
    res.render("user-forgot-password", {
      err: req.session.mobileNumber_OR_CountryCode_ERR,
      serverError: req.session.serverError,
    });
...
router.post("/otpverify", (req, res, next) => {
  ...
  twilio.verify
    ...
    .catch((err) => {
      console.log("RRR otp verify le error : ", err);
      req.session.serverError = true;
      res.redirect("/userForgotPassword");
    });
});
...
router.post("/placeOrder", async (req, res) => {
  if (req.session.loggedIn) {
    ...
    userHelpers
      .placeOrder(req.body, cartProduct, GrandTotal)
      .then((orderId) => {
        ...
        // Paypal
          ...
          userHelpers
            .generatePaypal(orderId, GrandTotal)
            .then((paySuccess) => {
              res.json(paySuccess);
            })
            .catch((err) => {
...
router.get("/userorderedProduct", async (req, res) => {
  if (req.session.loggedIn) {
    ...
    let userOrderedProducts = await userHelpers.FindUserOrderedProducts(
      req.session.user._id
    );

    res.render("UserOrderedProduct", {
      user: true,
      name: req.session.user.username,
      user: req.session.user,
      cartcount,
...
router.post("/BuynowPlaceOrder", async (req, res) => {
  if (req.session.loggedIn) {
    ...
    userHelpers
      .BuynowPlaceOrder(req.body, orderproduct, productprice)
      .then((orderId) => {
        if (req.body["payment"] == "COD") {
          res.json({ COD_success: true });
        } else if (req.body.payment == "Razorpay") {
          userHelpers
            .generateRazorpay(orderId, productprice)
            .then((response) => {
              res.json(response);
            })
            .catch((err) => {
        ...
        // Paypal
          ...
          userHelpers
            .generatePaypal(orderId, productprice)
            .then((paySuccess) => {
              res.json(paySuccess);
            })
            .catch((err) => {
...
router.get("/categoryDisplay/:category", async (req, res) => {
  if (req.session.loggedIn) {
    ...
    userHelpers.DisplayCategory(req.params.category).then((categoryItems) => {
      res.render("userCategoryDisplay", {
        user: true,
        cartcount,
        name: req.session.user.username,
        categoryItems,
