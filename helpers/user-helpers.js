...
module.exports = {
  ...
  signupOTPsuccess: (userdetail) => {
    return new Promise(async (resolve, reject) => {
      ...
      var data = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ _id: details.insertedId });
  ...
  loginWithOTP: (detail) => {
    return new Promise(async (resolve, reject) => {
      ...
      let user = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ mobile: detail.mobile, countryCode: detail.countryCode });
      if (user) {
        response.user = user;
        response.status = true;
        resolve(response);
  ...
  blockedUser: (userId) => {
    return new Promise(async (resolve, reject) => {
      var BLOCK_USER = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .update({ _id: objectId(userId) }, { $set: { block: "false" } });
  ...
  findNumber: (userNum) => {
    return new Promise(async (resolve, reject) => {
      userNumber = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ mobile: userNum.mobile, countryCode: userNum.countryCode });
      ...
      if (cartItems[0]) {
        for (key in cartItems) {
          product = await db
            .get()
            .collection(collection.PRODUCT_COLLECTION)
            .findOne({ _id: objectId(cartItems[key].item) });
          db.get()
            .collection(collection.CART_COLLECTION)
            .updateOne(
              { products: { $elemMatch: { item: objectId(product._id) } } },
              {
                $set: {
                  "products.$.newprice": product.newprice,
                  "products.$.totalprice":
                    cartItems[key].quantity * product.newprice,
                },
              }
            )
            .then(() => {
      ...
      if (detail.count == -1 && detail.quantity == 1) {
        ...
        db.get()
          ...
          .updateOne(
            {
              _id: objectId(detail.cart),
              "products.item": objectId(detail.product),
            },
            {
              $inc: {
                "products.$.quantity": detail.count,
                "products.$.totalprice": detail.price * detail.count,
              },
            }
          )
          .then((response) => {
  ...
  UnBlockedUser: (userId) => {
    return new Promise(async (resolve, reject) => {
      var BLOCK_USER = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .update({ _id: objectId(userId) }, { $set: { block: "true" } });
  ...
  addtoWishlist: (proId, userId) => {
    return new Promise(async (resolve, reject) => {
      let checkingWishlist = await db
        .get()
        .collection(collection.WISH_LIST_COLLECTION)
        .findOne({ proId: objectId(proId) });
  ...
  AddCategory: (detail) => {
    return new Promise(async (resolve, reject) => {
      var checkcategory = await db
        .get()
        .collection(collection.CATEGORY_COLLECTION)
        .findOne({ category: detail.category });

      if (checkcategory == null) {
        let categoryadded = await db
          .get()
          .collection(collection.CATEGORY_COLLECTION)
          .insertOne(detail);
    ...
    return new Promise((resolve, reject) => {
      db.get()
        ...
        .then((response) => {
          db.get()
            .collection(collection.PRODUCT_COLLECTION)
            .deleteMany({ category: categoryName });

          db.get()
            .collection(collection.CART_COLLECTION)
            .update({}, { $pull: { products: { category: categoryName } } });
  ...
  DeleteWishlist: (proId) => {
    return new Promise(async (resolve, reject) => {
      db.get()
        .collection(collection.WISH_LIST_COLLECTION)
        .deleteOne({ proId: objectId(proId) })
        .then((response) => {
          resolve(response);
        });
    });
  ...
  findAllorderStatus: () => {
    return new Promise(async (resolve, reject) => {
      let TotalbuynowOrder = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow" });
      let TotalCartOrder = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          {
      ...
      let buynowCancell = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Cancelled" });
      let cartCancell = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          { $match: { mode: "cart" } },
      ...
      let buynowDelivered = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Delivered" });
      let cartDelivered = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          { $match: { mode: "cart" } },
      ...
      let buynowPending = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Pending" });

      let cartPending = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          { $match: { mode: "cart" } },
      ...
      let buynowPlaced = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Placed" });
      ...
      let buynowShipped = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Shipped" });
  ...
  findpaymentMethod: () => {
    return new Promise(async (resolve, reject) => {
      let COD = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ paymentMethod: "COD" });
      let Razorpay = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ paymentMethod: "Razorpay" });
      let Paypal = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ paymentMethod: "Paypal" });
      let payment = {};
      payment.COD = COD;
  ...
  BuyNowcheckingCouponExist: (couponCode, productprice, userId) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (coupon) {
        let checkingalreadyUsingCoupon = await db
          .get()
          .collection(collection.ORDER_COLLECTION)
          .findOne({ userId: objectId(userId), coupon: couponCode });

        if (checkingalreadyUsingCoupon == null) {
          var couponSuccess = {};

          couponSuccess.discountprice =
            (productprice * coupon.couponOffer) / 100;
  ...
  BuynowFindSalesReport: () => {
    return new Promise(async (resolve, reject) => {
      let buynowSalesReport = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
  ...
  FindUserOrderedProducts: (userId) => {
    return new Promise(async (resolve, reject) => {
      let userorderedProduct = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .find({ userId: objectId(userId) })
        .sort({ date: -1 })
  ...
  categoryproductsTotal: (categoryDetails) => {
    return new Promise(async (resolve, reject) => {
      let total = await db
        .get()
        .collection(collection.PRODUCT_COLLECTION)
        .count({ category: categoryDetails });
  ...
  find7daysSalesReport: (day) => {
    return new Promise(async (resolve, reject) => {
      let buynowsales = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({ mode: "buyNow", status: "Delivered", date: day });
      let cartsales = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          { $match: { mode: "cart" } },
  ...
  FindTrendingProduct: (productName, proId) => {
    return new Promise(async (resolve, reject) => {
      let buynowTrendingProduct = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .count({
          mode: "buyNow",
      ...
      let cartTrendingProduct = await db
        .get()
        .collection(collection.ORDER_COLLECTION)
        .aggregate([
          { $match: { mode: "cart" } },
      ...
      var details = {};
      var totalTrendingProduct =
        buynowTrendingProduct + totalcartTrendingProduct;
